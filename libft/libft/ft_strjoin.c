...
/*   Updated: 2024/11/19 17:41:04 by ebengtss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoin_c(char *s1, char *s2, char c)
{
	...
	if (!s1)
		return (ft_strdup(s2));
	if (!s2)
		return (dst = ft_strdup(s1), free(s1), dst);
	len1 = ft_strlen(s1);
	len2 = ft_strlen(s2);
	is_c = 1;
	if (s1[len1 - 1] != c)
		is_c = 2;
	dst = malloc(sizeof(char) * (len1 + len2 + is_c));
	if (!dst)
		return (NULL);
	ft_strlcpy(dst, s1, len1 + 1);
	if (is_c == 2)
		dst[len1] = c;
	ft_strlcpy(&dst[len1 + is_c - 1], s2, len2 + 1);
	return (dst);
}
